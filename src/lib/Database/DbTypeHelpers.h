...
template<>
struct DbPrintableTypeBinding<char> {
	static void print(std::ostringstream& stream, void* data) {
		stream << (int)(*(reinterpret_cast<char*>(data)));
	}
};
...
template<>
struct RZU_EXTERN DbPrintableTypeBinding<SQL_TIMESTAMP_STRUCT> {
	static void print(std::ostringstream& stream, void* data);
};
...
template<int ARRAY_SIZE> struct DbTypeBinding<unsigned char[ARRAY_SIZE]> : public DbTypeBinding<char(&)[ARRAY_SIZE]> { };
